...
export default class App extends Component {
      ...
      await window.FBInstant.initializeAsync().then(async () => {
        await window.FBInstant.startGameAsync().then(async () => {
          ...
          if (contextType != "SOLO") {
            ...
            let gameDoc = await ref.doc(contextId).get();
            ...
            if (gameDoc.exists) {
              ...
              this.setState({ redirectTo: "game-room" });
            }
          }
        });
      });
  ...
  render() {
    console.log("redirec to", this.state.redirectTo);
    return (
      <Router>
        <div>
          <Switch>
            ...
            <Route path="/">
              <Home redirectTo={this.state.redirectTo} />
            </Route>
          </Switch>
        </div>
      </Router>
